...
// Schedule represents a train schedule, including its metadata and associated locations.
type Schedule struct {
	ID uint64 `gorm:"primaryKey"`
	// Trains are sets of schedules identified by a common UID. A schedule for a specific train service can be
	// uniquely identified by UID, Start Date and STP Indicator. (from https://wiki.openraildata.com/index.php?title=SCHEDULE)
...
// GetCompanyNameByATOC returns the company name associated with a given ATOC code. If the code is "ZZ", it returns a specific message indicating a non-passenger operator with an obfuscated name. If the code is not found in the predefined list, it returns a message indicating that the operator code is unknown.
...
/*
ApplyOverlays applies a list of overlay schedules to the current schedule based on matching criteria.

It checks if the schedule is eligible for overlays (not an STP schedule) and then iterates through the sorted overlays to find a match based on CIFTrainUID and the schedule's active date range.

If a matching overlay is found, it updates the current schedule's fields with the overlay's values, giving precedence to non-empty fields in the overlay. After applying an overlay, it augments the schedule with human-readable descriptions and computed fields.
*/
func (schedule *Schedule) ApplyOverlays(overlays []Schedule, datetime int64) {
	slog.Debug("applying overlays", "count", len(overlays), "overlays", overlays)

	if len(overlays) < 1 {
		return
	}

	// N - STP schedule: similar to a permanent schedule, but planned through the Short Term Planning
	// process and not capable of being overlaid
	...
	for _, overlay := range overlays {
		if !overlayApplied {
			slog.Debug("Testing overlay...", "ciftrainuid", overlay.CIFTrainUID, "startdatets", overlay.ScheduleStartDateTS, "enddatets", overlay.ScheduleEndDateTS, "date", datetime)
			if (overlay.CIFTrainUID == schedule.CIFTrainUID) && (overlay.ScheduleStartDateTS <= datetime && overlay.ScheduleEndDateTS > datetime) {
				slog.Debug("Applying overlay", "combinedid", overlay.CombinedID)
	...
	if overlayApplied {
		if err := schedule.AugmentSchedule(); err != nil {
			slog.Warn("There was an error augmenting the schedule", "error", err.Error())
		}
	} else {
		slog.Debug("No overlay applied")
	}
}

// AugmentSchedule enriches the schedule with human-readable descriptions and computed fields.
